            ...
            case PD_OOK_STATE_GAP_START:    // Beginning of gap - it might be a spurious gap
                ...
                // Or this gap is for real?
                    ...
                    if (fsk_pulses->num_pulses > PD_MIN_PULSES) {
                        // Store last pulse/gap. Both detectors leave the final
                        // pulse in flight at package end; without the wrap-up the
                        // min/max path silently drops the message's last bits,
                        // which fails checksum on messages that end in a pulse.
